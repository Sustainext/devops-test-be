...
def create_data(raw_response: RawResponse, table_name):
    for local_data in raw_response.data:
        ...
        for index, (age_group, value) in enumerate(age_group_and_value.items()):
            employment_object, _ = EmploymentTurnover.objects.update_or_create(
                ...
                defaults={
                    "value": value,
                    "employee_turnover_beginning": get_integer(local_data["beginning"]),
                    "employee_turnover_ending": get_integer(local_data["end"]),
                },
            )
            employment_object.save()


def employment_turnover_analysis(raw_response: RawResponse):
    path_slugs = {
