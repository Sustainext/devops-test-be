...
class CanadaBillReport:
    ...
    def __init__(self, user, organization, corporate, year: int):
        self.excel_file_url = default_storage.open(
            "canada_bill_s211/BillS211Template.xlsx"
        )
    ...
    def get_part_one_data(self):
        queryset = SubmissionInformation.objects.filter(
            organization__client=self.user.client,
            organization__in=self.user.orgs.all(),
        ).filter(
            organization=self.organization, corporate=self.corporate, year=self.year
        )
    ...
    def get_part_two_data(self):
        queryset = ReportingForEntities.objects.filter(
            organization__client=self.user.client,
            organization__in=self.user.orgs.all(),
        ).filter(
            organization=self.organization, corporate=self.corporate, year=self.year
    ...
    def retrieve_key2_if_key1_matches(
        self, dictionary: dict, key1: str, key2: str, value: str
    ):
        if dictionary.get(key1) == value:
            return dictionary.get(key2)
        return self.not_available

    def create_and_merge_rows(
        self, sheet: Worksheet, row_insert_number: int, insert_data: list
    ) -> int:
        if not insert_data:
        ...
        for idx, data in enumerate(insert_data[1:]):
            ...
            sheet.cell(
                row=current_row_index, column=target_column + 1
            ).border = thin_border
            sheet.cell(
                row=current_row_index, column=target_column + 1
        ...
        sheet.merge_cells(
            start_row=answer_cell.row,
            start_column=target_column,
            end_row=answer_cell.row + numbers_of_rows_to_insert - 1,
        ...
        for i in data:
            if i not in ignore_keys:
                ...
                temporary_list.append(
                    self.modify_text_as_per_requirements(
                        heading=heading, points=data[i]
                    )
                )
    ...
    def modify_part_one(self):
        ...
        part_one_sheet.cell(row=3, column=3).value = part_one_data.get(
            "screen1_q1", self.not_available
        )
        part_one_sheet.cell(row=4, column=3).value = part_one_data.get(
            "screen1_q2", self.not_available
        )
        part_one_sheet.cell(row=5, column=3).value = part_one_data.get(
            "screen1_q3", self.not_available
        )
        part_one_sheet.cell(
            row=6, column=3
        ).value = f"{part_one_data.get('screen1_to_q4', self.not_available)} - {part_one_data.get('screen1_form_q4', self.not_available)}"
        part_one_sheet.cell(row=7, column=3).value = part_one_data.get(
            "screen2_q1", self.not_available
        )
        part_one_sheet.cell(row=8, column=3).value = self.retrieve_key2_if_key1_matches(
            dictionary=part_one_data, key1="screen2_q1", key2="screen2_q2", value="Yes"
        )
        part_one_sheet.cell(row=9, column=3).value = self.html_to_plain_text(
            html_string=self.retrieve_key2_if_key1_matches(
                dictionary=part_one_data,
                key1="screen2_q1",
                key2="screen2_q3",
                value="Yes",
            )
        ...
        part_one_sheet.cell(row=10, column=3).value = part_one_data.get(
            "screen2_q4", self.not_available
        )
        part_one_sheet.cell(row=11, column=3).value = part_one_data.get(
            "screen3_q1", self.not_available
        )
        current_row = self.create_and_merge_rows(
            sheet=part_one_sheet,
            row_insert_number=12,
            insert_data=[
                i["legalName"]
                for i in part_one_data.get("screen3_q2", [self.not_available])
            ],
        )
        current_row = self.create_and_merge_rows(
            sheet=part_one_sheet,
            row_insert_number=current_row,
            insert_data=[
                i["businessNumber"]
                for i in part_one_data.get("screen3_q2", [self.not_available])
            ],
        )
        part_one_sheet.cell(row=current_row, column=3).value = part_one_data.get(
            "screen4_q1", self.not_available
        )
        current_row += 1
        current_row = self.create_and_merge_rows(
            sheet=part_one_sheet,
            row_insert_number=current_row,
            insert_data=self.add_in_list_if_not_null(
                original_list=part_one_data.get("screen4_q2", [self.not_available]),
                probable_null_item=part_one_data.get("screen4_q3_other"),
            ),
        )
        current_row = self.create_and_merge_rows(
            sheet=part_one_sheet,
            row_insert_number=current_row,
            insert_data=self.modify_recursive_list_data(
                part_one_data.get("screen5_q1")
            ),
        )
        current_row = self.create_and_merge_rows(
            sheet=part_one_sheet,
            row_insert_number=current_row,
            insert_data=self.add_in_list_if_not_null(
                original_list=self.modify_recursive_list_data(
                    data=part_one_data.get("screen6_q1"), ignore_keys=["other"]
                ),
                probable_null_item=part_one_data.get("screen6_q2"),
            ),
        )
        part_one_sheet.cell(row=current_row, column=3).value = part_one_data.get(
            "screen7_q1", self.not_available
        )
        current_row += 1
        part_one_sheet.cell(row=current_row, column=3).value = part_one_data.get(
            "screen7_q2", self.not_available
        )
    ...
    def modify_part_two(self):
        ...
        part_two_sheet.cell(row=3, column=3).value = part_two_data.get(
            "screen1_q1", self.not_available
        )
        current_row = self.create_and_merge_rows(
            sheet=part_two_sheet,
            row_insert_number=4,
            insert_data=part_two_data.get("screen1_q2", [self.not_available]),
        )
        current_row = self.create_and_merge_rows(
            sheet=part_two_sheet,
            row_insert_number=current_row,
            insert_data=part_two_data.get("screen2_q1", [self.not_available]),
        )
        part_two_sheet.cell(row=current_row, column=3).value = self.html_to_plain_text(
            html_string=part_two_data.get("screen2_q2", self.not_available)
        )
        part_two_sheet.cell(row=current_row, column=3).alignment = Alignment(
            wrap_text=True
        )
        current_row += 1
        part_two_sheet.cell(row=current_row, column=3).value = part_two_data.get(
            "screen3_q1", self.not_available
        )
        current_row += 1
        current_row = self.create_and_merge_rows(
            sheet=part_two_sheet,
            row_insert_number=current_row,
            insert_data=part_two_data.get("screen3_q2", [self.not_available]),
        )
        part_two_sheet.cell(
            row=current_row, column=3
        ).value = REPORTING_FOR_ENTITIES_RADIO_BUTTON[4].get(
            (part_two_data.get("screen4_q1", self.not_available)), self.not_available
        )
        part_two_sheet.cell(row=current_row, column=3).alignment = Alignment(
            wrap_text=True
        )
        ...
        current_row = self.create_and_merge_rows(
            sheet=part_two_sheet,
            row_insert_number=current_row,
            insert_data=part_two_data.get("screen4_q2", [self.not_available]),
        )
        current_row = self.create_and_merge_rows(
            sheet=part_two_sheet,
            row_insert_number=current_row,
            insert_data=self.add_in_list_if_not_null(
                original_list=self.modify_recursive_list_data(
                    data=part_two_data.get("screen5_q1"), ignore_keys="other"
                ),
                probable_null_item=part_two_data.get(
                    "screen5_q2", [self.not_available]
                ),
            ),
        )
        part_two_sheet.cell(row=current_row, column=3).value = self.html_to_plain_text(
            html_string=part_two_data.get("screen5_q3", self.not_available)
        )
        part_two_sheet.cell(row=current_row, column=3).alignment = Alignment(
            wrap_text=True
        )
        ...
        part_two_sheet.cell(
            row=current_row, column=3
        ).value = REPORTING_FOR_ENTITIES_RADIO_BUTTON[6].get(
            (part_two_data.get("screen6_q1", self.not_available)), self.not_available
        )
        part_two_sheet.cell(row=current_row, column=3).alignment = Alignment(
            wrap_text=True
        )
        ...
        current_row = self.create_and_merge_rows(
            sheet=part_two_sheet,
            row_insert_number=current_row,
            insert_data=part_two_data.get("screen6_q2", [self.not_available]),
        )
        part_two_sheet.cell(
            row=current_row, column=3
        ).value = REPORTING_FOR_ENTITIES_RADIO_BUTTON[7][0].get(
            (part_two_data.get("screen7_q1", self.not_available)), self.not_available
        )
        part_two_sheet.cell(row=current_row, column=3).alignment = Alignment(
            wrap_text=True
        )
        ...
        part_two_sheet.cell(
            row=current_row, column=3
        ).value = REPORTING_FOR_ENTITIES_RADIO_BUTTON[7][1].get(
            (part_two_data.get("screen7_q2", self.not_available)), self.not_available
        )
        part_two_sheet.cell(row=current_row, column=3).alignment = Alignment(
            wrap_text=True
        )
        ...
        part_two_sheet.cell(
            row=current_row, column=3
        ).value = REPORTING_FOR_ENTITIES_RADIO_BUTTON[7][2].get(
            (part_two_data.get("screen7_q3", self.not_available)), self.not_available
        )
        part_two_sheet.cell(row=current_row, column=3).alignment = Alignment(
            wrap_text=True
        )
        current_row += 1
        part_two_sheet.cell(row=current_row, column=3).value = part_two_data.get(
            "screen8_q1", self.not_available
        )
        current_row += 1
        current_row = self.create_and_merge_rows(
            sheet=part_two_sheet,
            row_insert_number=current_row,
            insert_data=part_two_data.get("screen8_q2", [self.not_available]),
        )
        part_two_sheet.cell(row=current_row, column=3).value = part_two_data.get(
            "screen8_q3", self.not_available
        )
    ...
    def insert_rows_in_sheet(
        self, sheet_name: str, start_row: int, num_rows_to_insert: int
    ):
        """
        ...
        try:
            ...
            logger.info(
                f"Inserted {num_rows_to_insert} rows into sheet '{sheet_name}' starting before row {start_row}."
            )
        except KeyError:
    ...
    def generate_excel_report_data(self):
        ...
        self.excel_file.active = self.excel_file.sheetnames.index(
