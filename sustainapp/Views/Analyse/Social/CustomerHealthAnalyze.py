...
class CustomerHealthAnalyzeView(APIView):
    ...
    def process_customer_health(self, path, filter_by):
        ...
        req_data = {"Q1": "", "Q2": "", "Q3": ""}
        temp_req_data_2 = req_data
        for i in cust:
            temp_req_data_2[i.metric_name] = i.value
        if temp_req_data_2["Q1"] == "No":
            pass
        ...
        if not cust or temp_req_data_2["Q1"] == "No":
            logger.info(
                f"Customer Health Analyze : There is no data for the organization {self.org} and the corporate {self.corp} and the path {path} and the year {self.year}"
            )
