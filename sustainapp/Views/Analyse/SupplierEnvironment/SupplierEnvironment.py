...
class SupplierEnvAnlayzeView(APIView):
    ...
    def calculate_percentage(self, q1, q2=None, filter_for_corp=False):
        """Calculate the percentage of the given values. filter_for_corp when true indicates that the user has selected to view by Organization, But the organization has no data, So we are checking for it's corporates"""
    ...
    def new_suppliers(self, path, filter_by, filter_for_corp=False):
        ...
        if (
            ...
            """When the if condition is true, it means that path should have 2 fields named Q1 and Q2. 
            We calculate the percentage using them, it any one field is not added we return empty list"""
            try:
                q1 = sup_env_data.data[0]["Q1"]
                q2 = sup_env_data.data[0]["Q2"]
                lst.append(
                    self.calculate_percentage(
                        float(q1), float(q2), filter_for_corp=filter_for_corp
                    )
                )
            ...
            """When this elif expression is true, we only have one field (Q1) in the data, so the other field which is required to calculate the percentage is the self.suppliers_assesed."""
            try:
                self.suppliers_assesed = (
            ...
            """If there is no data when the Organization alone is selected, then we enter this case.
            Here we loop through all it's Corporates to get the data"""

            """Here we make self.corp to True as a break condition for the recurssion,
            before we get out of this looping through all the corporates we will make self.corp to None
            which would be it's initial state. By this the next path can check if it's Organization has
            no data, So that it should check for it's corporates."""
            self.corp = True
            ...
            for corporate in corps_of_org:
                ...
                """Pass an additional parameter to tell the calculate_percentage function that  we are getting the corporates of the Organization since no data was found in the organization""" 
    ...
    def get(self, request):
        ...
        try:
            ...
            supplier_env_data["gri_308_2e"] = self.new_suppliers(
                "gri-supplier_environmental_assessment-negative_environmental-308-2e",
                filter_by,
            )
